...
export default function Hero() {
  ...
  return (
    <section ref={sectionRef} id="hero" className="relative overflow-hidden" style={{ height: '110vh' }}>
      ...
      <div className="relative z-10 h-full flex flex-col justify-end" style={{ paddingBottom: 'clamp(4rem, 12vh, 10rem)' }}>
        <div className="space-gutter">
          <div ref={titleRef} className="max-w-5xl" style={{ perspective: '800px' }}>
            <div className="overflow-hidden"><h1 className="hero-line font-serif text-cream-50 fluid-display" style={{ fontWeight: 700 }}>Una cucina</h1></div>
            <div className="overflow-hidden"><h1 className="hero-line font-serif italic text-cream-300 fluid-display" style={{ fontWeight: 400, marginLeft: 'clamp(0.5rem, 3vw, 4rem)' }}>ricca di</h1></div>
            <div className="overflow-hidden"><h1 className="hero-line font-serif text-cream-50 fluid-display" style={{ fontWeight: 800 }}>semplicità.</h1></div>
          </div>
          <p className="hero-sub text-cream-200/70 max-w-lg" style={{ marginTop: 'clamp(1.5rem, 4vh, 3rem)', marginLeft: 'clamp(0rem, 2vw, 3rem)', fontSize: 'clamp(0.95rem, 1.2vw, 1.2rem)', lineHeight: 1.75, fontWeight: 300 }}>
          ...
          <div className="hero-cta flex flex-col sm:flex-row gap-6 sm:gap-10" style={{ marginTop: 'clamp(2rem, 5vh, 3.5rem)', marginLeft: 'clamp(0rem, 2vw, 3rem)' }}>
            ...
            <a href="#cucina" className="text-cream-200/50 text-sm tracking-wider uppercase hover:text-cream-100 transition-colors duration-500 self-center" style={{ fontWeight: 300 }}>Scopri il menu ↓</a>
          </div>
        </div>
        <div ref={smallImgRef} className="hidden lg:block absolute right-[8%] bottom-[22%] w-52 h-64 rounded-lg overflow-hidden shadow-2xl" style={{ transform: 'rotate(3deg)' }}>
          <img src={tagliolino} alt="Tagliolino San Michele" className="w-full h-full object-cover" />
          <div className="absolute bottom-0 left-0 right-0 p-3" style={{ background: 'linear-gradient(transparent, rgba(74,47,27,0.8))' }}>
            <span className="mono text-[9px] text-cream-200/80 tracking-widest uppercase">Piatto iconico</span>
          </div>
        </div>
